...
let tests =
    testList (
        ...
        [ test ("self returns a pid", fun _ ->
          ...
          test ("sendAfter and cancelTimer", fun _ ->
          ...
          test ("register and whereis", fun _ ->
          ...
          test ("date returns valid year month day", fun _ ->
          ...
          test ("dateYear dateMonth dateDay match date", fun _ ->
                  let (year, month, day) = Erlang.date ()
          ...
          test ("time returns valid hour minute second", fun _ ->
          ...
          test ("localtime returns valid date and time", fun _ ->
          ...
          test ("universaltime returns valid date and time", fun _ ->
          ...
          test ("cancelTimer returns None for invalid ref", fun _ ->
          ...
          test ("sendAfterTo sends to specific pid", fun _ ->
                  let pid = Erlang.self ()
          ...
          test ("byteSize returns correct size", fun _ ->
          ...
          test ("atomToList returns charlist not binary", fun _ ->
          ...
          test ("tail returns rest of list", fun _ ->
                  let xs: BeamList<int> = emitErlExpr () "[1, 2, 3]"
          ...
          test ("head raises on empty list", fun _ ->
                  let empty: BeamList<int> = emitErlExpr () "[]"
                  assertThat (fun () -> Erlang.head empty |> ignore) throws
                  )

          test ("tail raises on empty list", fun _ ->
                  let empty: BeamList<int> = emitErlExpr () "[]"
                  assertThat (fun () -> Erlang.tail empty |> ignore) throws
                  )

          test ("Atom.ofString builds a real atom, not a binary", fun _ ->
