...
let tests =
    testList (
        "Dynamic",
        [ test ("Decode.int succeeds on integer", fun _ ->
          ...
          test ("Decode.int fails on non-integer", fun _ ->
          ...
          test ("Decode.string succeeds on binary", fun _ ->
                  let d: Dynamic = emitErlExpr () "<<\"hello\">>"
          ...
          test ("Decode.atom succeeds on atom", fun _ ->
          ...
          test ("Decode.bool succeeds on true", fun _ ->
          ...
          test ("Decode.field extracts map value", fun _ ->
          ...
          test ("Decode.field errors on missing key", fun _ ->
          ...
          test ("Decode.list decodes homogeneous list", fun _ ->
          ...
          test ("Decode.list short-circuits on first decode error", fun _ ->
          ...
          test ("Decode.optional returns None for undefined", fun _ ->
          ...
          test ("Decode.optional returns Some for value", fun _ ->
                  let d: Dynamic = emitErlExpr () "42"
                  match Decode.optional Decode.int d with
          ...
          test ("Decode.tuple2 decodes a pair", fun _ ->
          ...
          test ("Decode.field with Atom.ofString key finds an atom-keyed field", fun _ ->
          ...
          test ("Decode combinators accept a plain lambda decoder", fun _ ->
