...
let tests =
    testList (
        "Port",
        [ test (
              "port streams a complete line and exit status",
              fun _ ->
          ...
          test (
              "port delivers an incomplete line when the process ends mid-line",
              fun _ ->
          ...
          test (
              "port receive leaves unrelated mailbox messages behind",
              fun _ ->
                  ...
                  let port =
                      match
                          startAbsolute "/bin/sh" (options [ "-c"; "read line; printf '%s\n' \"$line\"; exit 0" ] 128)
                      with
                      | Ok port -> port
                      | Error reason -> failwithf "could not start port fixture: %s" reason
          ...
          test (
              "port startAbsolute rejects a relative path",
              fun _ ->
                  ...
                  | Error reason ->
                      assertThat
                          reason
                          (isEqualTo
                              "path must be absolute, maxLineLength must be positive, and stderrToStdout requires useStdio")
                  | Ok _ -> failwith "expected an error for a relative path"
          )

          test (
              "port startOnPath errors when the executable is missing",
              fun _ ->
          ...
          test (
              "port rejects a non-positive maxLineLength",
              fun _ ->
                  ...
                  | Error reason ->
                      assertThat
                          reason
                          (isEqualTo "maxLineLength must be positive and stderrToStdout requires useStdio")
                  | Ok _ -> failwith "expected an error for maxLineLength 0"
          )

          test (
              "port options redirect stderr and apply child environment",
              fun _ ->
          ...
          test (
              "port lifecycle operations are non-throwing after exit",
              fun _ ->
                  let port =
          ...
          test (
              "port foldMessages keeps trailing oversized JSONL fragments after exit",
              fun _ ->
                  let port =
                      match
                          startAbsolute "/bin/sh" (options [ "-c"; "printf '{\\\"value\\\":\\\"0123456789\\\"}'" ] 8)
                      with
                      | Ok port -> port
                      | Error reason -> failwithf "could not start port fixture: %s" reason
          ...
          test (
              "monitored port emits a typed down notification",
              fun _ ->
                  let port =
                      match startAbsolute "/bin/sh" (options [ "-c"; "exit 0" ] 128) with
                      | Ok port -> port
