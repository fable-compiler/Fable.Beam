...
let tests =
    testList (
        "Re",
        [ test (
              "isMatch returns true for matching pattern",
              fun _ -> assertThat (isMatch "hello world" "hello") (isTrue)
          )

          test (
              "isMatch returns false for non-matching pattern",
              fun _ -> assertThat (isMatch "hello world" "xyz") (isFalse)
          )

          test ("isMatch with digit pattern", fun _ -> assertThat (isMatch "abc123" "\\d+") (isTrue))

          test ("isMatch anchored no match", fun _ -> assertThat (isMatch "hello" "^world") (isFalse))

          test (
              "compile returns Ok for valid pattern",
              fun _ ->
          ...
          test (
              "compile returns Error for invalid pattern",
              fun _ ->
          ...
          test (
              "isMatchMP with compiled pattern",
              fun _ ->
          ...
          test (
              "isMatchMP with compiled pattern no match",
              fun _ ->
                  match compile "\\d+" with
          ...
          test (
              "run returns Some with whole match at index 0",
              fun _ ->
          ...
          test (
              "run returns Some with capture groups",
              fun _ ->
          ...
          test ("run returns None for no match", fun _ -> assertThat (run "hello world" "xyz") (isEqualTo None))

          test (
              "runMP returns captures for compiled pattern",
              fun _ ->
                  match compile "(\\d+)" with
                  | Ok mp ->
          ...
          test (
              "replaceFirst replaces only first occurrence",
              fun _ -> assertThat (replaceFirst "aabbaa" "a+" "X") (isEqualTo "Xbbaa")
          )

          test (
              "replaceFirstWith caseless replaces first case-insensitively",
              fun _ -> assertThat (replaceFirstWith "Aabbaa" "a+" "X" [ caseless ]) (isEqualTo "Xbbaa")
          )

          test (
              "replaceAll replaces all occurrences",
              fun _ -> assertThat (replaceAll "aabbaa" "a+" "X") (isEqualTo "XbbX")
          )

          test (
              "replaceAll with digit pattern",
              fun _ -> assertThat (replaceAll "abc123def456" "\\d+" "N") (isEqualTo "abcNdefN")
          )
          ...
          test (
              "splitParts limits result count",
              fun _ ->
          ...
          test (
              "isMatchWith caseless option matches different case",
              fun _ ->
          ...
          test (
              "isMatchWith multiline option matches after newline",
              fun _ ->
          ...
          test (
              "isMatchWith unicode option handles multi-byte characters",
              fun _ ->
          ...
          test (
              "runWith caseless returns original-case captures",
              fun _ ->
          ...
          test (
              "compileWith caseless produces case-insensitive MP",
              fun _ ->
          ...
          test (
              "replaceAllWith caseless replaces all cases",
              fun _ -> assertThat (replaceAllWith "Hello HELLO hello" "hello" "X" [ caseless ]) (isEqualTo "X X X")
          )

          test (
              "splitWith caseless splits on either case",
              fun _ ->
          ...
          test (
              "replaceFirstMP with compiled pattern",
              fun _ ->
          ...
          test (
              "replaceAllMP with compiled pattern",
              fun _ ->
                  match compile "\\d+" with
          ...
          test (
              "run with empty subject and optional group returns Some empty",
              fun _ ->
                  match run "" "a*" with
          ...
          test (
              "splitRaw returns the native list form of split",
              fun _ ->
                  let parts: BLists.BeamList<string> = splitRaw "a1b2c" "[0-9]"
