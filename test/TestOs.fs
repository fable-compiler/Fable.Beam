...
let tests =
    testList (
        "Os",
        [ test ("getenv returns None for unset var", fun _ ->
                  assertThat (getenv "FABLE_BEAM_TEST_UNSET_12345") (isEqualTo None))

          test ("putenv and getenv roundtrip", fun _ ->
          ...
          test ("cmd runs a command", fun _ ->
                  let result = cmd "echo hello"
                  assertThat result (isEqualTo "hello\n"))

          test ("systemTimeSeconds returns positive", fun _ ->
                  let t = systemTimeSeconds ()
                  assertThat ((t > 0)) (isTrue))

          test ("systemTime with TimeUnit returns a sensible value", fun _ ->
